...
module "db_username" {
  source     = "./modules/parameter-store"
  param_name = "/${var.name}/db_username"
  type       = "String"
...
module "admin_user" {
  source     = "./modules/parameter-store"
  param_name = "/${var.name}/admin_user"
  type       = "String"
...
module "admin_pass" {
  source     = "./modules/parameter-store"
  param_name = "/${var.name}/admin_pass"
  type       = "SecureString"
...
module "admin_email" {
  source     = "./modules/parameter-store"
  param_name = "/${var.name}/admin_email"
  type       = "String"
...
module "int_vpc_endpoint_sg" {
  source = "./modules/security-group"
  name   = "${var.name}-int-vpc-endpoint-sg"
  vpc_id = module.vpc.vpc_id

  create_ingress_cidr    = true
  ingress_cidr_block     = ["0.0.0.0/0"]
  ingress_cidr_from_port = [443]
  ingress_cidr_to_port   = [443]
...
module "asg_sg" {
  ...
  create_ingress_sg          = true
  ingress_security_group_ids = [module.alb_sg.id]
...
module "rds" {
  ...
  db_name                = module.db_name.value
  db_username            = module.db_username.value
  db_pass                = module.db_pass.value
